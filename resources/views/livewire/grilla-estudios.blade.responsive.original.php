<div>
    <div class="p-5 h-screen bg-gray-100">
        ...
        {{-- LA TABLA                           --}}
        {{-- *********************************   --}}
        <div class="overflow-x-auto overflow-y-auto rounded-lg shadow hidden md:block">
            <table id="estudios" class="w-full">
                <thead class="bg-gray-50 border-b-2 border-gray-200">
                    <tr>
                        ...
                        <th class="w-32 p-3 text-sm font-semibold tracking-wide text-left">Nombre</th>
                        ...
                        <th class="w-24 p-3 text-sm font-semibold tracking-wide text-left">Diag</th>
                        {{-- <th class="w-24 p-3 text-sm font-semibold tracking-wide text-left">Desc</th> --}}
                        <th class="w-32 p-3 text-sm font-semibold tracking-wide text-left">Ubic</th>
                        {{-- <th class="w-24 p-3 text-sm font-semibold tracking-wide text-left">PCuerpo</th> --}}
                        <th class="w-24 p-3 text-sm font-semibold tracking-wide text-left">I</th>
                        <th class="w-32 p-3 text-sm font-semibold tracking-wide text-left">Informe</th>
                        <th class="w-24 p-3 text-sm font-semibold tracking-wide text-left">Acc1</th>
                        <th class="w-24 p-3 text-sm font-semibold tracking-wide text-left">Acc2</th>
                        <th class="w-32 p-3 text-sm font-semibold tracking-wide text-left">Link</th>
                    </tr>
                </thead>
                ...
                @foreach ($paginator as $estudio)
                    <div class="bg-white space-y-3 p-4 rounded-lg shadow">
                        <div class="flex items-center space-x-2 text-sm">
                            ...
                            @if (isset($estudio['00080061']['Value'][0]))
                                @if (isset($estudio['00080061']['Value'][0]) and $estudio['00080061']['Value'][0] == 'DOC')
                                    <div class="text-gray-500">
                                        {{ $estudio['00080061']['Value'][1] }}</div>
                                @else
                                    <div class="text-gray-500">
                                        {{ $estudio['00080061']['Value'][0] }}</div>
                                @endif
                            @else
                                <div>-</div>
                            @endif
                            ...
                            @if (isset($estudio['00080020']['Value'][0]))
                                <div class="text-sm text-gray-700">
                                    {{ date('d/m/Y', strtotime($estudio['00080020']['Value'][0])) }}</div>
                            @else
                                <div>-</div>
                            @endif
                            ...
                            @if (isset($estudio['series'][0]['00081040']['Value'][0]))
                                <div class="text-sm text-gray-700">
                                    {{ $estudio['series'][0]['00081040']['Value'][0] }}</div>
                            @elseif (isset($estudio['series'][0]['00081050']['Value'][0]['Alphabetic']))
                                <div class="text-sm text-gray-700">
                                    {{ $estudio['series'][0]['00081050']['Value'][0]['Alphabetic'] }}</div>
                            @else
                                <div>-</div>
                            @endif

                            <div class="text-sm whitespace-normal px-4 py-2 text-gray-700">
                                <i class="fas fa-square-2-stack"></i>
                                <a href="#"
                                    wire:click="copiarTexto('Estudio de {!! $estudio['00100010']['Value'][0]['Alphabetic'] !!} de fecha {{ date('d/m/Y', strtotime($estudio['00080020']['Value'][0])) }}: http://imagenes.simedsalud.com.ar:8484/viewer.html?studyUID={{ $estudio['0020000D']['Value'][0] }}&serverName=Antartida')">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        ...
                        <div class="flex items-center space-x-2 text-sm">
                            ...
                            @if (isset($estudio['00100010']['Value'][0]['Alphabetic']))
                                <div class="text-sm text-black">
                                    {!! $estudio['00100010']['Value'][0]['Alphabetic'] !!}</div>
                            @else
                                <div>-</div>
                            @endif
                            ...
                            @if (isset($estudio['00100030']['Value'][0]))
                                <div class="text-sm text-gray-700">
                                    {{ date('d/m/Y', strtotime($estudio['00100030']['Value'][0])) }}</div>
                            @else
                                <div class="text-sm text-gray-700">-</div>
                            @endif --}}
                        </div>
                        ...
                        <div class="flex items-center space-x-2 text-sm">
                            ...
                            @if (isset($estudio['00080090']['Value'][0]['Alphabetic']))
                                <div class="text-sm text-gray-700">
                                    {{ $estudio['00080090']['Value'][0]['Alphabetic'] }}</div>
                            @else
                                <div class="text-sm text-gray-700">-</div>
                            @endif
                        ...
                        @if (isset($estudio['00201208']['Value'][0]))
                            <div class="p-3 text-sm text-gray-700 whitespace-nowrap">
                                {{ $estudio['00201208']['Value'][0] }}</div>
                        @else
                            <div class="p-3 text-sm text-gray-700 whitespace-nowrap">-</div>
                        @endif
                        --}}
                        <div class="flex items-center space-x-2 text-sm">
                            @if (array_search('DOC', $estudio['00080061']['Value']) != '')
                            ...
                            @if (isset($estudio['0020000D']['Value'][0]))
                                <div>
                                    <div class="text-sm whitespace-normal px-4 py-2">
                                        <a href="http://imagenes.simedsalud.com.ar:8484/viewer.html?studyUID={{ $estudio['0020000D']['Value'][0] }}&serverName=Antartida"
                                            class="inline-block rounded bg-green-600 px-4 py-2 text-sm text-white hover:bg-green-700"
                                            target="_blank">
                                            Ver
