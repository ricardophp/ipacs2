                        ...
                        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @if ($data)
                                        ...
                                        <tr>
                                            ...
                                            @if (isset($estudio['00080020']['Value'][0]))
                                                <td class="whitespace-nowrap px-4 py-2 text-gray-700">
                                                    {{ date('d/m/Y', strtotime($estudio['00080020']['Value'][0])) }}
                                                </td>
                                            @else
                                                <td>-</td>
                                            @endif
                                    ...
                                    @endforeach
                                @else
                                    <td class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">'No existen
                                        estudio cargados con su DNI. Si cree que es un error comuníquese con el área
                                        diagnóstico por Imágenes Sanatorio Antártida'</td>
                                @endif
                            </tbody>
                        </table>
                    @else
                        <th class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">No hay estudios para mostrar
                        </th>
                    @endif
                    @if ($data->hasPages())
                        <div class="mt-4">
                    ...
                    <div class="mb-4 bg-white space-y-3 p-4 border-gray-200 rounded-lg shadow">
                        <div class="flex items-center border-gray-200 space-x-2 text-sm">
                            ...
                            <div class="text-gray-500">
                                ...
                                @if (isset($estudio['00080020']['Value'][0]))
                                    <a
                                        class="whitespace-nowrap px-4 py-2 text-gray-700 font-bold">{{ date('d/m/Y', strtotime($estudio['00080020']['Value'][0])) }}</a>
                                @else
                                    <a>-</a>
                                @endif
                            </div>
                            <div>
                                {{-- doctor --}}
                                @if (isset($estudio['00080090']['Value'][0]['Alphabetic']))
                                    <a class="whitespace-nowrap px-4 py-2 text-gray-700 font-bold">Médico:
                                        {{ $estudio['00080090']['Value'][0]['Alphabetic'] }}</a>
                                @else
                                    <a>-</a>
                                @endif
                            </div>
                        ...
                        <div>
                            @if (isset($estudio['00081030']['Value'][0]))
                                <a class="whitespace-nowrap px-4 py-2 text-gray-700">Diagnóstico:
                                    {{ $estudio['00081030']['Value'][0] }}</a>
                            @else
                                <a>-</a>
                            @endif
                        </div>
                        <div class="text-sm font-medium text-black">
                            @if (isset($estudio['0020000D']['Value'][0]))
                                ...
                                <a class="text-sm whitespace-normal px-4 py-2 text-gray-700">
                                    <i class="fas fa-square-2-stack"></i>
                                    <a href="#"
                                        wire:click="copiarTexto('Estudio de {!! $estudio['00100010']['Value'][0]['Alphabetic'] !!} de fecha {{ date('d/m/Y', strtotime($estudio['00080020']['Value'][0])) }}: http://{{ config('api.url_medicview') }}/viewer.html?studyUID={{ $estudio['0020000D']['Value'][0] }}&serverName={{ config('api.server_medicview') }}')">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none"
...
@endif
@else
<td class="whitespace-nowrap px-4 py-2 font-medium text-gray-900">'No existen estudio cargados con su DNI. Si cree que
    es un error comuníquese con el área diagnóstico por Imágenes Sanatorio Antártida'</td>
@endif
